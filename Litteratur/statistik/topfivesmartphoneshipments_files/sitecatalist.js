...
/* test 1 SiteCatalyst code version: H.24.3.
...
var s_account=""
if(document.location.host.indexOf('idc.com')>-1){
    s_account="idc-global,idc-prod";
}
if(document.location.host.indexOf('idc-gi.com')>-1){
    s_account="idc-global,idc-gi,idc-insights";
}
if(document.location.host.indexOf('idc-ei.com')>-1){
    s_account="idc-global,idc-ei,idc-insights";
}
if(document.location.host.indexOf('idc-fi.com')>-1){
    s_account="idc-global,idc-fi,idc-insights";
}
if(document.location.host.indexOf('idc-hi.com')>-1){
    s_account="idc-global,idc-hi,idc-insights";
}
if(document.location.host.indexOf('idc-mi.com')>-1){
    s_account="idc-global,idc-mi,idc-insights";
}
if(document.location.host.indexOf('idc-ri.com')>-1){
    s_account="idc-global,idc-ri,idc-insights";
}
if(document.location.host.indexOf('devcl.idc.com')>-1){
    s_account="idcdev";
}
...
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx,dmg,qz,bin"
s.linkInternalFilters="javascript:,idc.,idc-ei.,idc-fi.,idc-gi.,idc-hi.,idc-mi.,idc.ri.,idctracker."
s.linkLeaveQueryString=false
s.linkTrackVars="prop7,prop8,eVar1,eVar2"
...
function s_doPlugins(s) {
    ...
    if(!s.pageType && !s.pageName){
        ...
        s.queryVarsList="containerId"     // query parameters to keep
        s.pathExcludeDelim=";" // portion of the path to exclude
        s.pathConcatDelim="/"   // page name component separator
        s.pathExcludeList="index"   // elements to exclude from the path
        s.pageName=s.getPageName();
    }
    if(s.pageName){
        if(s.pageName.toLowerCase().indexOf('getdoc.jsp')>-1){
            s.pageName=(document.title);
        }

        if(document.title.indexOf("Not found (404)") > -1){
            s.pageName = "404:" +document.location;
            s.pageType = "errorPage";
        }}


    /* s.channelExtract Example: 1.0 */
    ...
    if(!s.hier1) s.hier1=s.channelExtract("/",10,s.pageName,0);
    ...
    if(!s.eVar3){
        s.eVar3 = s.getQueryParam('containerId')

        if(s.eVar3.indexOf('PRF')>-1){
            s.eVar5='Analyst';
        }
        if(s.eVar3.indexOf('IDC_P')>-1){
            s.eVar5='Factsheet';
        }
        if(s.eVar3.indexOf('pr')>-1){
            s.eVar5='Press Release';
        }
        else
            s.eVar5='Research';
    ...
    s.prop13=s.eVar13=s.getVisitNum();
    s.prop14=s.eVar14=s.getNewRepeat(30,'s_nr');
    s.prop15=s.eVar15=s.getDaysSinceLastVisit('s_lv');

    /* Time Parting */
    s.prop16=s.eVar16=s.getTimeParting('h','-8')
    s.prop17=s.eVar17=s.getTimeParting('d')
    s.prop18=s.eVar18=s.getTimeParting('w')



    /*
     s.LargeBannerClick=function(){
     if (document.getElementById('ss_img')
     if (document.getElementById('ss_img')!= null){
     s.eVar1=document.getElementById('ss_img').value}

     }


     s.deletefromcart=function(){
     if (document.getElementById('ss_img_Small')){
     var s=s_gi(s_account);
     s.linkTrackVars='eVar2';
     s.eVar2=document.getElementById('ss_img_Small').src;
     s.tl(this,'o','Small Banner Click')}
     }
     */



    s.SmallBannerClick=function(){
        if (document.getElementById('ss_img_Small')){
            var s=s_gi(s_account);
            s.linkTrackVars='eVar2';
            s.eVar2=document.getElementById('ss_img_Small').src;
    ...
    if(s.pageName.indexOf('searchresults')>-1){
        if(document.getElementById('keyword1') != null){
            s.prop5=document.getElementById('keyword1').value
            s.eVar4=s.prop5;
        }
    ...
    if(s.pageName.indexOf('searchresults')>-1){
        if(document.getElementById('crumbspan2').innerHTML!='undefined'){
            var nsr = document.getElementById('crumbspan2');
            if(nsr.innerHTML.indexOf('0-0')>-1){
                s.prop6='D=v4';
    ...
    /* Internal Keyword Search Number ID*/
    if(!s.eVar9){s.eVar9 = s.getQueryParam('sid')};
    s.prop9=s.eVar9;


    /* Internal Search Filter*/

    if(!s.eVar19){s.eVar19 = s.getQueryParam('cmp')};
    if(!s.eVar20){s.eVar20 = s.getQueryParam('athr')};
    if(!s.eVar21){s.eVar21 = s.getQueryParam('cg')};
    if(!s.eVar22){s.eVar22 = s.getQueryParam('geo')};
    if(!s.eVar23){s.eVar23 = s.getQueryParam('top')};
    if(!s.eVar23){s.eVar23 = s.getQueryParam('vm')};
    if(!s.eVar23){s.eVar23 = s.getQueryParam('src')};
    ...
    if(!s.campaign)
        s.campaign=(s.getQueryParam('uid'),'s_campaign',0);
    s.eVar6=s.getQueryParam('cmtype');
    s.eVar7=s.getQueryParam('pa');




    /* Do not refire search event if the same search term passed in twice */
    var t_search=s.getValOnce(s.eVar4,'s_stv',0);
    if(t_search=='')
    {
    ...
    if(s.pageName=='US/IDC/action/register.do'){
        s.events=s.apl(s.events,'event5',',',1);
        s.events=s.getValOnce(s.events,'s_event5',0);
    }
    /*if(s.pageName=='US/IDC/action/register.do')
     = s.getQueryParam('success')
     {
     s.events=s.apl(s.events,'event6',',',1);
     s.events=s.getValOnce(s.events,'s_event6',0);
     }*/

//product variables and events
    if(s.pageName.indexOf('Document at a Glance')>-1){
        s.events=s.apl(s.events,'prodView',',',1);
        s.products = ";" + s.eVar3
    }
    if(s.pageName.indexOf('process_cart_add.jsp')>-1){
        s.events=s.apl(s.events,'scAdd',',',1);
        s.products = ";" + s.eVar3
    }
    if(document.title.indexOf("Shopping Cart") > -1){
        s.events=s.apl(s.events,'scOpen',',',1);
        //s.products = ";" + s.eVar3
    }
    if(document.title.indexOf("Payment Information") > -1){
        s.events=s.apl(s.events,'event8',',',1);
        s.products = ";" + s.eVar3
    }
    if(document.title.indexOf("Confirm Purchase") > -1){
        s.events=s.apl(s.events,'scCheckout',',',1);
        s.products = ";" + s.eVar3;;;; + eVar5
    }
    if(document.title.indexOf("Purchase Approved") > -1){
        s.events=s.apl(s.events,'purchase',',',1);
        s.products = ";" + s.eVar3;;;; + s.eVar5
    }
    /*if(document.referrer.indexOf('Payment Information')>-1){
     s.events=s.apl(s.events,'purchase',',',1);
     s.products = ";" + s.eVar3;;;; + s.eVar5
     }
     ...
     s.tl(this,'o','Delete from Shopping Cart')}
     }
     */



    if(location.href.indexOf("5_1282") > -1){
        s.prop7="Document Type- Briefs"
    }
    if(location.href.indexOf("5_1283") > -1){
        s.prop7="Document Type- Analysts"
    }
    if(location.href.indexOf("5_1285") > -1){
        s.prop7="Document Type- Event Proceedings"
    }
    if(location.href.indexOf("5_1287") > -1){
        s.prop7="Document Type- IDC Links"
    }
    if(location.href.indexOf("5_1288") > -1){
        s.prop7="Document Type- Insights-Only Research"
    }
    if(location.href.indexOf("5_1289") > -1){
        s.prop7="Document Type- Pivot Table"
    }
    if(location.href.indexOf("5_1290") > -1){
        s.prop7="Document Type- Presentations"
    }
    if(location.href.indexOf("5_1291") > -1){
        s.prop7="Document Type- Press Releases"
    }
    if(location.href.indexOf("5_1293") > -1){
        s.prop7="Document Type- Top 10 Predictions"
    }
    if(location.href.indexOf("5_1294") > -1){
        s.prop7="Document Type- Studies"
    }
    if(location.href.indexOf("5_1804") > -1){
        s.prop7="Document Type- IDC MarketScape"
    }
    if(location.href.indexOf("5_1805") > -1){
        s.prop7="Document Type- Market Analysis Perspective"
    }
...
function _DD(v){return (typeof v != _hud) ? 1 : 0}
function _EV(a, b, c, d) {if(typeof a.window != 'object'){
    if (a.addEventListener) {a.addEventListener(b, c, false)} else if (a.attachEvent) {a.attachEvent(((d == 1) ? '' : 'on') + b, c)}}}
function _IC(a, b, c) {return a.charAt(b) == c ? 1 : 0}
function _IF(a, b, c) {return a.lastIndexOf(b, c ? c : _IL(a))}
function _II(a, b, c) {return a.indexOf(b, c ? c : 0)}
function _IL(a) {return a != _hud ? a.length : 0}
function _IP(a, b){return a.split(b)}
function _IS(a, b, c){return b > _IL(a) ? '' : a.substring(b, c != null ? c : _IL(a))}
function _LD(a, b, c, d, e, f) {b = a.pathname, d = '', e = '';b = _IS(b, _IF(b, '/') + 1, _IL(b));
    c = (_DD(_dlf)) ? _dlf : '';if (c != 'n') {if (_II(c, '!') > -1) {d = ',' + _IS(c, 0, _II(c, '!'));
        e = ',' + _IS(c, _II(c, '!') + 1, _IL(c))} else {d = ',' + c}}f = _II(b, '?');b = (f > -1) ? _IS(b, 0, f) : b;
    if (_IF(b, '.') > -1) {f = _IS(b, _IF(b, '.'), _IL(b));if (_II(_dl + d, f) > -1 && _II(e, f) < 0) {var dl = b;
        if (_DD(_dft)) {if (_dft == 'y' && a.name) {dl = a.name} else if (_dft == 'full') {dl = a.pathname;
            if (!_IC(dl, 0, '/')) dl = '/' + dl}}return dl}}}
function _LE(a, b, c, d, e, f, g, h, i, j, k, l) {
    b = '([0-9A-Za-z\\-]*\\.)', c = location.hostname, d = a.href, h = '', i = '';eval('__f=/' + b + '*' + b + '/');
    if (_DD(__f)) {__f.exec(c);j = (_DD(_elf)) ? _elf : '';if (j != 'n') {if (_II(j, '!') > -1) {
        h = _IS(j, 0, _II(j, '!'));i = _IS(j, _II(j, '!') + 1, _IL(j))} else {h = j}}k = 0;if (_DD(_elf) && _elf != 'n') {
        if (_IL(i)) {l = _IP(i, ',');for (g = 0; g < _IL(l); g++) if (_II(d, l[g]) > -1) return}
        if (_IL(h)) {l = _IP(h, ',');for (g = 0; g < _IL(h); g++) if (_II(d, l[g]) > -1) k = 1}}
        if (_II(a.hostname, RegExp.$2) < 0 || k) {e = _IL(d) - 1;return _IC(d, e, '/') ? _IS(d, 0, e) : d}}}
function _LI(a){var b = "" + a.innerHTML,bu = _TL(b),i = _II(bu, "<img");if (bu && i > -1) {
    eval("__f=/ src\s*=\s*['\"]?([^'\" ]+)['\"]?/i");__f.exec(b);if (RegExp.$1) b = RegExp.$1}return b}
...
function _LS(a, b, c, d, e, f, g) {c = _D('lim') ? _lim : 100;b = (_IL(a) > c) ? _A(_IS(a, 0, c)) : _A(a);
    b = _LSP(b, '%0A', '%20');b = _LSP(b, '%0D', '%20');b = _LSP(b, '%09', '%20');c = _IP(b, '%20');d = _NA();
    e = 0;for (f = 0; f < _IL(c); f++) {g = _RP(c[f], '%20', '');if (_IL(g) > 0) {d[e++] = g}}b = d.join('%20');
    return unescape(b)}
function _LSP(a, b, c, d) {d = _IP(a, b);return d.join(c)}
function _LT(e) {_lc=true;if ((e.which && e.which == 1) || (e.button && e.button == 1)){
    var a = document.all ? window.event.srcElement : this;
    if(a.getAttributeNode('onclick')!=null && (a.getAttributeNode('onclick').value.indexOf('_hbLink')>-1||a.innerHTML.indexOf('_hbLink')>-1)){
        return false;} for (var i = 0; i < 4; i++) {if (a.tagName && _TL(a.tagName) != 'a' && _TL(a.tagName) != 'area') {
        a = a.parentElement}};var b = _LN(a),c = '',d = '';a.lid = b[0];a.lpos = b[1];if (_D('lt') && _lt != 'manual') {
        if ((a.tagName && _TL(a.tagName) == 'area')) {if (!_IL(a.lid)) {if (a.parentNode) {
            if (a.parentNode.name) a.lid = a.parentNode.name;else a.lid = a.parentNode.id}}if (!_IL(a.lpos)) a.lpos = a.coords
        } else {if (_IL(a.lid) < 1) a.lid = _LS(a.text ? a.text : a.innerHTML ? a.innerHTML : '');
            if (!_IL(a.lid) || _II(_TL(a.lid), '<img') > -1) a.lid = _LI(a)}}
    if (!_IL(a.lpos) && _D('lt') == 'auto_pos' && a.tagName && _TL(a.tagName) != 'area') {c = document.links;
        for (d = 0; d < _IL(c); d++) {if (a == c[d]) {a.lpos = d + 1;break}}}var _f = 0,j = '',k = '',
        l = (a.protocol) ? _TL(a.protocol) : '';_clt = 'o';
    a.lid=a.lid.indexOf('<')>-1?a.lid.replace(/<\/?[^>]+(>|$)/g, ''):a.lid;
    if (l && l != 'mailto:' && l != 'javascript:') {j = _LE(a), k = _LD(a);if (_DD(k)) a.fn = k;else if (_DD(j)) a.el = j}
    if (_D('lt') && _IC(_lt, 0, 'n') != 1 && _DD(a.lid) && _IL(a.lid) > 0) {_f = 1}if (_DD(a.fn)) {_clt = 'd';_f = 2}
    else if (_DD(a.el)) {_clt = 'e';_f = 1}if (_f > 0) {s.prop7=a.lid.replace(/\+/g, ' ');
        s.prop7=s.prop7.replace(/&(nbsp|amp|quot|lt|gt);/g, "");
        s.prop7=s.prop7.replace(/\s{2,}/g, ' ');
        s.prop7=s.prop7.replace(/[\||\]|\[|\{|\}|\#|\$|\%|\^|\*|\!|\<|\>|\~|\;]/g, '');
        s.prop8=s.pageName+"|"+s.prop7;s.prop35=s.pageName;s.prop36=a.lpos;s.tl(true,_clt,s.prop7);}}}

function _LV(a, b, c) {b = _LN(a);c = b[0] + b[1];if (_IL(b[0])) {_lvid += _A(b[0]) + ",";_lvpos += _A(b[1]) + ",";_lvl += c}}
function _LVP(a, b, c, d, e) {c = _II(a, '&' + b + '=');c = c < 0 ? _II(a, '?' + b + '=') : c;if (c > -1) {
    d = _II(a, '&', c + _IL(b) + 2);e = _IS(a, c + _IL(b) + 2, d > -1 ? d : _IL(a));if (!_ec) {if (!(_II(e, '//') == 0)) return e
...
function _TL(a) {return a.toLowerCase()}
function __X(a) {if (_ec == 0) {_ec = 1;a = document;if (_NN('lt') || _NN('dlf') || _NN('elf')) _LP(a.links);
    if (_NN('lt') && _IL(_lvl) > 0 && _lvl != -1) {}}}
_lt = 'auto';_lvid = _lvpos = _lvl = '';_ec = _if = _ll = _hec = _hfs = _hfc = _hfa = _ic = _pC = _fc = _pv = 0;_lvm = 300;
_lidt = 'lid';_lpost = 'lpos';_pum = _erf = _hif = 0;_hud = 'undefined';_dlf = '!.do';_dft = 'n';_elf = 'n';
_dl = '.'+ s.linkDownloadFileTypes.split(',').join('.,');_huf = 'function';_EV(window, 'load', __X);eval("setTimeout('__X()',3000)");
...
s.visitorNamespace="idc"
s.trackingServer="idc.d2.sc.omtrdc.net"
